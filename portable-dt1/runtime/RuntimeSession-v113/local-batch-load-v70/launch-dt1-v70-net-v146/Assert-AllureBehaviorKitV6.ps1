...
function Assert-AllureBehaviorKitV6 {
 ...
 $taskPins=@{
  'allure-dt1-launch-v70-net-v146\manifest.json'='32FCAC4F22F2AC07461081189561E08C920A00FAC24888DAF52845B8CAB5ECEE'
  ...
  'launch-v146\osrs-endpoint-manifest-v4.json'='588616DAF7143302DD0050B23F641237EB2809E461C388DDFE1CA3248E8F7FB5'
 }
 ...
 $taskCurrent=Get-Content -LiteralPath (Join-Path $taskCase 'allure-dt1-launch-v70-net-v146\manifest.json') -Raw|ConvertFrom-Json
 ...
 foreach($f in $taskCurrent.Files){if((Get-FileHash -LiteralPath (Join-Path ($taskWork+'\launch-dt1-v70-net-v146') $f.Name) -Algorithm SHA256).Hash -ne $f.SHA256){throw 'Current launch script changed'}}
 ...
 $taskNet=Get-Content -LiteralPath (Join-Path $taskCase 'launch-v146\osrs-network-verification-v4.json') -Raw|ConvertFrom-Json
 $taskEndpoints=Get-Content -LiteralPath (Join-Path $taskCase 'launch-v146\osrs-endpoint-manifest-v4.json') -Raw|ConvertFrom-Json
