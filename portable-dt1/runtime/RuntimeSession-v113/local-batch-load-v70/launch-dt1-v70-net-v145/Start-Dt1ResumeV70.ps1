...
try {
 ...
 if($ValidateOnly){$taskResult.Mode='validate-only';$taskResult.Passed=$true;$taskResult.Stage='validated'}
  ...
  if($ObserveRun){$taskResult.Mode='observe-existing';$taskResult.Run=$ObserveRun}
   ...
   if([long](Get-CimInstance Win32_OperatingSystem).FreePhysicalMemory -lt 2000000){throw 'Guest memory preflight'}
   ...
   $taskStarter=Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList @('-NoProfile','-NonInteractive','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File','C:\SandboxGuard\local-batch-load-v70\launch-dt1-v70-net-v145\Start-LocalBatchConsoleV70.ps1') -WindowStyle Hidden -PassThru
   ...
   if(Test-Path -LiteralPath $taskPath){
    ...
    if($taskText.Trim()){
     ...
     if($taskState.Phase -eq 'running' -and $taskState.PluginList -eq 'registered-stopped' -and $taskState.BatchPlugins -eq 'registered-stopped'){
      ...
      if((Get-CimInstance Win32_Process -Filter ('ProcessId='+$taskGame.Id)).ParentProcessId -ne $taskHarness.Id){throw 'Game parent identity'}
...
}catch{$taskResult.ErrorType=$_.Exception.GetType().FullName;$taskResult.Stage='refused-or-unconfirmed'}
