...
$caseRoot='C:\SandboxLocalLogs132\first-launch-20260906-v1'
$target=Join-Path 'C:\SandboxLocalLogs132\plugin-batch-20260906-v1' ('local-batch-vm-v70-'+$Run+$(if($AfterAttachment){'-after'}else{'-before'})+'.json')
if(Test-Path -LiteralPath $target) {throw 'Existing validation must not be overwritten.'}
$profilePath='C:\SandboxLocalLogs132\resource-export-20260906-v3\current-vm-profile.json'
...
try {
    ...
    . 'C:\SandboxGuard\local-batch-load-v70\launch-dt1-v70-net-v145\LocalBatchSessionGateV4.ps1'
    ...
    $parent=(Get-CimInstance Win32_Process -Filter ('ProcessId='+$game.Id)).ParentProcessId
    ...
    $kitPath=Join-Path $caseRoot 'local-test-kit-v1\manifest.json'
    if((Get-FileHash -LiteralPath $kitPath -Algorithm SHA256).Hash -ne 'C61205321E0DF60C3C32E64C3C62CFDE1BDA38CC1A3CD64B78DA16AA202361A4'){throw 'Runtime kit manifest pin'}
