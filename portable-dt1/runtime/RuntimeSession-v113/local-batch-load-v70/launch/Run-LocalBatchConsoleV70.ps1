...
function Save-State {$state|ConvertTo-Json -Depth 4|Set-Content -LiteralPath $statePath -Encoding UTF8}
try{
    Save-State
    ...
    $watcher=Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList @('-NoProfile','-NonInteractive','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File','C:\CanaryTools\NextPluginsV1\Watch-LocalBatchConsoleV4.ps1','-SupervisorId',$PID,'-SupervisorStartTicks',$self.StartTime.ToUniversalTime().Ticks,'-Run',$Run) -WindowStyle Hidden -PassThru
    $watchFile=Join-Path $case ($Run+'-watchdog.json')
    for($i=0;$i -lt 40 -and !(Test-Path -LiteralPath $watchFile);$i++){Start-Sleep -Milliseconds 100}
    if(!(Test-Path -LiteralPath $watchFile) -or !(Get-Content -LiteralPath $watchFile -Raw|ConvertFrom-Json).Ready){throw 'Cleanup watchdog not ready'}
    & 'C:\CanaryTools\NextPluginsV1\Activate-OsrsForLocalV4.ps1' -Run $Run
    ...
    $harness=Start-Process -FilePath (Join-Path $work 'GameLoadTestV2.exe') -ArgumentList '--local-connected-test' -WorkingDirectory $work -NoNewWindow -PassThru -RedirectStandardOutput $stdout -RedirectStandardError $stderr
    $state.HarnessId=$harness.Id;$state.HarnessStartTicks=$harness.StartTime.ToUniversalTime().Ticks;$state.Phase='running';Save-State
    ...
    while(!$harness.HasExited){
        while(!$reader.EndOfStream){
            ...
            if(Update-PluginListLaunchGate $listGate $line){
                ...
                if($game.Path -ne (Join-Path $work 'osclient.exe') -or $identity.ParentProcessId -ne $harness.Id){throw 'Local list target is not this harness child'}
                ...
                $listWorker=Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList @('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File','C:\CanaryTools\FirstLaunchV1\Attach-LocalPluginList.ps1','-Tag',$Run,'-ExpectedProcessId',$game.Id,'-ExpectedStartTicks',$game.StartTime.ToUniversalTime().Ticks) -WindowStyle Hidden -PassThru
            }
        }
        ...
        if(!$batchWorker -and !$batchReported -and ([DateTime]::UtcNow-$launchStartedUtc).TotalSeconds -gt 240){throw 'Core/JVM/Mike startup deadline reached'}
