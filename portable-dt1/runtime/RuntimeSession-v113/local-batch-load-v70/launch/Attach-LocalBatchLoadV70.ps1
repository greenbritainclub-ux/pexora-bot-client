...
try{
 Save-BatchAttach
 foreach($name in $taskPins.Keys){if((Get-FileHash -LiteralPath (Join-Path $taskWork $name) -Algorithm SHA256).Hash -ne $taskPins[$name]){throw 'V70 helper pin'}}
 ...
 & 'C:\CanaryTools\NextPluginsV1\Test-LocalBatchSessionGateV4.ps1' *> (Join-Path $taskCase ($Run+'-session-fixture.txt'))
 ...
 & powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File 'C:\SandboxGuard\local-batch-load-v70\launch\Validate-LocalBatchVmV70.ps1' -Run $Run -ExpectedProcessId $ExpectedProcessId -ExpectedStartTicks $ExpectedStartTicks
 ...
 & powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File 'C:\SandboxGuard\local-batch-load-v70\launch\Validate-LocalBatchVmV70.ps1' -Run $Run -ExpectedProcessId $ExpectedProcessId -ExpectedStartTicks $ExpectedStartTicks -AfterAttachment
