...
function Assert-AllureBehaviorKitV6 {
 ...
 $taskPins=@{
  'allure-dt1-launch-v70\manifest.json'='DD71620A62BEAE0116D7748D627D34B1AA5C006E3C4062F936B910FACD63E32F'
  ...
  'launch-v54\osrs-endpoint-manifest-v4.json'='DEA50F21348163B04FFBD78831F401C412F3BC644411657D4920E5C9E6127508'
 }
 ...
 $taskCurrent=Get-Content -LiteralPath (Join-Path $taskCase 'allure-dt1-launch-v70\manifest.json') -Raw|ConvertFrom-Json
 ...
 foreach($f in $taskCurrent.Files){if((Get-FileHash -LiteralPath (Join-Path ($taskWork+'\launch-dt1-v70') $f.Name) -Algorithm SHA256).Hash -ne $f.SHA256){throw 'Current launch script changed'}}
 ...
 $taskNet=Get-Content -LiteralPath (Join-Path $taskCase 'launch-v54\osrs-network-verification-v4.json') -Raw|ConvertFrom-Json
 $taskEndpoints=Get-Content -LiteralPath (Join-Path $taskCase 'launch-v54\osrs-endpoint-manifest-v4.json') -Raw|ConvertFrom-Json
