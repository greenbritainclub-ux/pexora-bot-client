...
try{
    ...
    & 'C:\SandboxGuard\local-batch-load-v70\launch-dt1-v70-net-v155\Activate-OsrsForLocalV4.ps1' -Run $Run
    ...
    while(!$harness.HasExited){
        while(!$reader.EndOfStream){
            ...
            if(Update-PluginListLaunchGate $listGate $line){
                ...
                $parentId=Get-PexoraParentProcessIdV155 $game.Id
                if($game.Path -ne (Join-Path $work 'osclient.exe') -or $parentId -ne $harness.Id){throw 'Local list target is not this harness child'}
        ...
        # The V73 fixture selects stale rebuilt test archives. Do not register these
        # rows until their canonical capture, API, and lifecycle mappings are verified.
