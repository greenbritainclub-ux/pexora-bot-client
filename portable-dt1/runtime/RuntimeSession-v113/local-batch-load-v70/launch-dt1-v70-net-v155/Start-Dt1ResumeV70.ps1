...
. (Join-Path $PSScriptRoot 'LocalProcessInfoV155.ps1')
...
try {
 ...
 & (Join-Path $PSScriptRoot 'Test-ClientUiLaunchGateV9.ps1')
 if($ValidateOnly){$taskResult.Mode='validate-only';$taskResult.Passed=$true;$taskResult.Stage='validated'}
  ...
  if($ObserveRun){$taskResult.Mode='observe-existing';$taskResult.Run=$ObserveRun}
   ...
   if((Get-PexoraAvailablePhysicalMemoryV155) -lt 2048000000){throw 'Guest memory preflight'}
   ...
   # Starter stdout/stderr are captured for diagnosis; the hidden process itself
   # writes its session report as the launch gate.
   $taskStamp=[DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss-fff')
   $taskStarter=Start-Process -FilePath 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' -ArgumentList @('-NoProfile','-NonInteractive','-WindowStyle','Hidden','-ExecutionPolicy','Bypass','-File','C:\PreparedRuntime\local-batch-load-v70\launch-dt1-v70-net-v155\Start-LocalBatchConsoleV70.ps1') -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $taskFirst ('starter-'+$taskStamp+'-out.txt')) -RedirectStandardError (Join-Path $taskFirst ('starter-'+$taskStamp+'-err.txt'))
   ...
   if(Test-Path -LiteralPath $taskPath){
    ...
    if($taskText.Trim()){
     ...
     if($taskState.Phase -eq 'running' -and $taskState.PluginList -eq 'registered-stopped' -and $taskState.BatchPlugins -eq 'registered-stopped'){
      ...
      if((Get-PexoraParentProcessIdV155 $taskGame.Id) -ne $taskHarness.Id){throw 'Game parent identity'}
...
}catch{$taskResult.ErrorType=$_.Exception.GetType().FullName;$taskResult.Error=$_.Exception.Message;$taskResult.ErrorPosition=$_.InvocationInfo.ScriptName+':'+$_.InvocationInfo.ScriptLineNumber;$taskResult.ErrorStack=$_.ScriptStackTrace;$taskResult.Stage='refused-or-unconfirmed'}
