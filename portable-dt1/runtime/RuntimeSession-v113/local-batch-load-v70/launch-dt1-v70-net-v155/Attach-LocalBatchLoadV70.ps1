...
try{
 ...
 & 'C:\SandboxGuard\local-batch-load-v70\launch-dt1-v70-net-v155\Test-LocalBatchSessionGateV4.ps1' *> (Join-Path $taskCase ($Run+'-session-fixture.txt'))
 ...
 & powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File 'C:\SandboxGuard\local-batch-load-v70\launch-dt1-v70-net-v155\Validate-LocalBatchVmV70.ps1' -Run $Run -ExpectedProcessId $ExpectedProcessId -ExpectedStartTicks $ExpectedStartTicks
 ...
 $required=@("SUMMARY`trequested=4`tregistered=4`trows=4`tdefinitionFailures=0`tregistrationFailures=0",'RESULT=0','PLUGIN_START_REQUESTED=false','MIKE_ROW_PRESERVED=true;EXISTING_RUNNING_SET_UNCHANGED=true;LIST_REBUILT_ON_EDT=true')
 ...
 & powershell.exe -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File 'C:\SandboxGuard\local-batch-load-v70\launch-dt1-v70-net-v155\Validate-LocalBatchVmV70.ps1' -Run $Run -ExpectedProcessId $ExpectedProcessId -ExpectedStartTicks $ExpectedStartTicks -AfterAttachment
