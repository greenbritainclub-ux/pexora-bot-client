...
function Assert-AllureBehaviorKitV6 {
 ...
 foreach($f in $taskCurrent.Files){if((Get-FileHash -LiteralPath (Join-Path ($taskWork+'\launch-dt1-v70-net-v99') $f.Name) -Algorithm SHA256).Hash -ne $f.SHA256){throw 'Current launch script changed'}}
