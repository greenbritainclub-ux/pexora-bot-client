...
static bool exception(JNIEnv* e){
 ...
 jthrowable problem=e->ExceptionOccurred();e->ExceptionClear();std::string type="unknown";
 jclass problemType=problem?e->GetObjectClass(problem):nullptr,jclassType=e->FindClass("java/lang/Class");
 if(problemType&&jclassType){jmethodID name=e->GetMethodID(jclassType,"getName","()Ljava/lang/String;");if(name){jstring value=(jstring)e->CallObjectMethod(problemType,name);if(value){const char* text=e->GetStringUTFChars(value,nullptr);if(text){type=text;e->ReleaseStringUTFChars(value,text);}}}}
 if(e->ExceptionCheck())e->ExceptionClear();
 status((std::string("JNI_EXCEPTION_TYPE=")+type).c_str());
 return true;
}
static int inspect(JNIEnv* e){
 ...
 jstring path=e->NewStringUTF("file:/C:/PreparedRuntime/delve-dt1-v1/test-v55/DelveTestV55.jar");if(exception(e)||!path)return 52;jobject u=e->NewObject(url,ctor,path);if(exception(e)||!u)return 53;
 ...
 jstring name=e->NewStringUTF("ai.pexora.localboot.DelveNoStartV55");if(exception(e)||!name)return 62;jclass helper=(jclass)e->CallObjectMethod(loader,load,name);if(exception(e)||!helper)return 63;
...
static DWORD WINAPI worker(void*){
 wchar_t filename[256];swprintf_s(filename,L"C:\\CanaryLogs\\delve-dt1-v1\\delve-test-native-v55-%lu.txt",GetCurrentProcessId());if(GetFileAttributesW(filename)!=INVALID_FILE_ATTRIBUTES)return 67;report.open(filename,std::ios::out|std::ios::binary);if(!report)return 41;
 if(!scoped()){status("REFUSED=scope");report<<"RESULT=40\n";report.flush();return 40;}
